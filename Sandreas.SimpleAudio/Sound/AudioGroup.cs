...
public class AudioGroup : AudioNode
{
    ...
    public AudioGroup Parent
    {
        ...
        set
        {
            if (this == Default)
            {
                throw new InvalidOperationException("Unable to set the parent of the default (root) audio group.");
            }
    ...
    protected override void PopulateBuffer(Span<float> output)
    {
        lock (_sources)
        {
            ...
            foreach (var node in _sourcesRemove)
            {
                _sources.Remove(node);
            }

            foreach (var node in _sourcesAdd)
            {
                _sources.Add(node);
            }
        ...
        lock (_sources)
        {
            ...
            if (!IsMixerGraphAcyclic())
            {
                throw new InvalidOperationException(
                    "Audio graph must be a DAG. Adding this node will introduce a cycle.");
            }
        }
    }
    ...
    private bool IsMixerGraphAcyclic()
    {
        ...
        {
            if (statusTable.TryGetValue(node, out var status))
            {
                return status;
            }
        ...
        bool IsMixerGraphAcyclic(AudioNode node)
        {
            ...
            if (node is AudioGroup group)
            {
                foreach (var child in group.Children)
                {
                    ...
                    if (GetStatus(child) == STATUS_ACTIVE)
                    {
                        return false;
                    }
