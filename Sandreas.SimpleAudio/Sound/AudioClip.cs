...
public sealed class AudioClip
{
    ...
    private static short[] DecodeFiniteStream(Stream stream)
    {
        ...
        if (decoder.Length > 0)
        {
            ...
            // todo [sandreas]: decoder.Length is ulong, span uses int cast, what the fuck?
            var read = decoder.Decode(new Span<short>(samples, 0, (int)decoder.Length));
            // todo [sandreas]: ulong vs int
            if (read != (ulong)decoder.Length)
            {
                throw new InvalidOperationException(
                    $"Error when decoding, read {read} samples but expected {decoder.Length}.");
            }
        ...
        else
        {
            ...
            {
                // Allocate next block
                var block = new short[BLOCK_SIZE * (ulong)AudioAdapter.Channels];

                // Read samples into block
                // todo [sandreas]: ulong / int conversion is not good!
                count = decoder.Decode(new Span<short>(block, 0, (int)BLOCK_SIZE));
                if (count > 0)
                {
                    ...
                    if (total >= (int.MaxValue - count))
                    {
                        ...
                        throw new InvalidOperationException(
                            $"Error when decoding, too many samples! Stream may be infinite length.");
                    }
            ...
            var samples = new short[total * (ulong)AudioAdapter.Channels];
            ...
            foreach (var block in blocks)
            {
                ...
                if (i == (blocks.Count - 1))
                {
                    // todo [sandreas]: ulong to int conversion
                    blockSize = (int)(total * (ulong)AudioAdapter.Channels) % block.Length;
                }
