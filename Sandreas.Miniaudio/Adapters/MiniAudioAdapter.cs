using Sandreas.Miniaudio.Decoders;
...
public sealed unsafe class MiniAudioAdapter : AudioAdapter
{
    private readonly void* _device;
    private readonly DataProcessCallback _dataProc;

    public MiniAudioAdapter(int sampleRate, bool enableAudioCapture, IOutputMixer? outputMixer = null)
        : base(sampleRate, enableAudioCapture, outputMixer)
    {
    ...
    private void DataProcessCallback(void* pDevice, void* pOutput, void* pInput, uint frameCount)
    {
        ...
        {
            OnMicrophoneInput(new Span<short>(pInputSamples, sampleCount));
        }

        if (pOutputSamples != null)
        {
            OnSpeakerOutput(new Span<short>(pOutputSamples, sampleCount));
        }
    }
    ...
    public override IAudioDecoder CreateDecoder(Stream stream)
    {
