...
namespace Sandreas.Miniaudio.Adapters;
...
public abstract class AudioAdapter : IDisposable
{
    ...
    private readonly IOutputMixer? _outputMixer;
    ...
    /// <param name="outputMixer">IOutput mixer (e.g. AudioGroup.Default.OutputMixer</param>
    protected internal AudioAdapter(int sampleRate, bool enableAudioCapture, IOutputMixer? outputMixer = null)
    {
        if (sampleRate <= 0)
        {
            throw new InvalidOperationException("Sample rate must greater or equal to 1.");
        }

        _outputMixer = outputMixer;
    ...
    /// </summary>
    public static AudioAdapter Instance { get; set; }
    ...
    internal void Initialize()
    {
        if (Instance != null)
        {
            throw new InvalidOperationException(
                $"Unable to initialize a second instance of {nameof(AudioAdapter)}. Dispose the first instance.");
        }
    ...
    public abstract IAudioDecoder CreateDecoder(Stream stream);
    ...
    protected internal void OnSpeakerOutput(Span<short> output)
    {
        ...
        if (_buffer.Length < output.Length)
        {
            Array.Resize(ref _buffer, output.Length);
        }
    ...
    private void MixOutput(Span<float> buffer)
    {
        _outputMixer?.MixOutput(buffer);
    }
    ...
    protected internal void OnMicrophoneInput(Span<short> input)
    {
        ...
        if (_buffer.Length < input.Length)
        {
            Array.Resize(ref _buffer, input.Length);
        }
