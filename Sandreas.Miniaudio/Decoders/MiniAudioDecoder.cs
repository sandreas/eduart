...
using Sandreas.Miniaudio.NativeWrappers;
using Sandreas.Miniaudio.NativeWrappers.Enums;
using static Sandreas.Miniaudio.NativeWrappers.MiniAudio;
namespace Sandreas.Miniaudio.Decoders;
    ...
    internal sealed unsafe class MiniAudioDecoder : IAudioDecoder
    {
        ...
        private readonly MiniAudio.DecoderReadCallback _readProc;
        private readonly MiniAudio.DecoderSeekCallback _seekProc;
        private byte[] _readBuffer;
        private readonly Stream _stream;
        
        private readonly MiniAudioDecoderConfig _config;
        ...
        public MiniAudioDecoder(Stream stream, MiniAudioDecoderConfig? conf = null)
        {
            _readBuffer = new byte[1024];
                
            _config = conf ?? new MiniAudioDecoderConfig();
            _stream = stream;

            // We want signed 2 channel 16 bit audio
            var config = ma_decoder_config_init(SampleFormat.S16,  _config.Channels,  _config.SampleRate);
        ...
        private ulong ReadBytes(void* _, void* pWriteBuffer, ulong bytesToRead)
        {
            if (_stream.CanRead)
            {
                ...
                if (_readBuffer.Length < size) { Array.Resize(ref _readBuffer, size); }
        ...
        private int SeekBytes(void* _, int byteOffset, SeekOrigin origin)
        {
            if (_stream.CanSeek)
            {
                if (origin == SeekOrigin.Current) { _stream.Seek(byteOffset, System.IO.SeekOrigin.Current); }
        ...
        /// </summary>
        public ulong Decode(Span<short> samples)
        {
            var count = samples.Length;

            fixed (short* pSamples = samples)
            {
                return ma_decoder_read_pcm_frames(_decoder, pSamples, (ulong) (count / _config.Channels)) * _config.Channels;
            }
        }
        ...
        {
            var result = ma_decoder_seek_to_pcm_frame(_decoder, offset);
